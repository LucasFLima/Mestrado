...
qualidade da especificação, o que se dá por meio de contratos bem definidos, é
fundamental.
...
O objetivo dessa seção é investigar o uso de construções de
...
serviços REST. Em outras palavras: responder ao objetivo geral desse trabalho de
pesquisa.

\subsection{Planejamento do estudo empírico}

Há diversas formas e técnicas de se conduzir um estudo empírico, cada uma com
...
um estudo em tempo real dos fenômenos. Porém, elas geram uma fase de análise
difícil, pois geram muitas informações e requerem um elevado conhecimento para
se ter interpretar corretamente.
...
caracterizasse situações em que se aplicaria \designbycontract{}. 

Essas ações foram modeladas conforme a técnica GQM\cite{basili1992software} e,
após avaliação dos benefícios e riscos, descartadas em razão de demandarem tempo
excessivo em suas etapas de preparação e execução. Como resultado, adotou-se
então a técnica da aplicação de questionário, pois, embora exija cuidado e
reflexão sobre as perguntas e respostas, é mais rápido de se preparar e permite
a participação de pessoas em tempos e locais distintos.
...
apresenta como as metas são estruturadas em GQM, com a finalidade de
caracterizá-la objetivamente em termos de objeto, finalidade, foco, ponto de
...
\neoidl{} estruturada conforme o método GQM é apresentado na Tabela
\ref{TabelaMetasGQM}.
...
ETAPAS
GQM é divido em seis etapas onde se deve:
• Desenvolver um conjunto metas organizacionais e metas de medições associadas para produtividade e da
qualidade;
• Gerar perguntas (baseadas em modelos) que definem os objetivos tão completamente quanto possível, em
uma forma quantificável.;
• Especificar as medidas necessárias para se obter as respostas a essas questões e acompanhar os processos e
produtos em conformidade com os objetivos;
• Desenvolver mecanismos para coleta de dados;
• Coletar, validar e analisar o dado em tempo real para prover retorno ao projeto para ação corretiva;
• Analisar o dado para avaliar a conformidade com as metas e formular recomendações para futuras
melhorias.
...
adapted from [17][18]. For each of the three main constructs
“perceived usefulness” (Ui), “ease of use” (Ei), and
...
using a particular system would be free of effort.” This follows from the definition of “ease”: “freedom from difficulty or
great effort”. A tool that is easy to use is more likely to be accepted by users.

A score is assigned to each response and the scores belonging to a particular
concept are combined so that subjects with the most favorable attitude will have the highest concept score while subjects with the least
...
going to use it in the future (self-predicted future usage). This serves us as
an indicator whether users prefer WIPS for inspection data collection rather than paper-based forms. For measuring self-predicted
...
\cite{laitenberger1998evaluating}
