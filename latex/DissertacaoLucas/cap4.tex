...
A proposta deste trabalho é estender os níveis de garantias, de modo a promover
...
incluir chamadas a funções, estendendo a validações a lógicas mais sofisticadas
...
\begin{figure}[htb]
\begin{small}
\lstinputlisting[language=NeoIDL,firstnumber=1]{trechos_codigo/livro_pre_basica.neo}
\end{small}
...
\begin{figure}[htb]
\begin{small}
\lstinputlisting[language=NeoIDL,firstnumber=1]{trechos_codigo/livro_pos_basica.neo}
\end{small}
...
\method{POST} do serviço principal, o serviço \emph{reserva.getSituacaoLivro} é acionado. Caso
esse serviço retorne o código diferente de \emph{HTTP
Unauthorized}, a operação \method{POST} é executada. Caso contrário, o serviço principal retorna o valor
...
\begin{figure}[htb] 
\begin{small} 
\lstinputlisting[language=NeoIDL,firstnumber=1]{trechos_codigo/livro_pre_servico.neo}
\end{small}
...
o processamento do \method{DELETE}, se a reserva foi efetivamente apagada por
...
\begin{figure}[htb] 
\begin{small} 
\lstinputlisting[language=NeoIDL,firstnumber=1]{trechos_codigo/livro_pos_servico.neo}
\end{small}
...
A sintaxe da \neoidl{} foi estendida para admitir a vinculação de pré e
...
A operação \method{GET} possui uma precondição em que o valor do atributo
...
A operação \method{POST} possui duas precondições (linhas 14 e 15). A primeira,
...
Na operação \method{DELETE}, a pós-condição faz a chamada ao serviço
\emph{reserva.consultaReserva}. Ou seja, se a operação \method{delete} executou
...
\begin{figure}[htb]
\begin{small} 
\lstinputlisting[language=NeoIDL,firstnumber=1]{trechos_codigo/livro_dbc_complete.neo}
\end{small}
...
arquitetura que estende a arquitetura base do \twisted{} \emph{Reactor},
...
pós-condição básica, que testa se o valor de resposta \emph{reserva.codigo} é
diferente de vazio (linhas 3 a 5). Caso a pós-condição não seja satisfeita, o
serviço retorna o código \emph{HTTP No Content}.
