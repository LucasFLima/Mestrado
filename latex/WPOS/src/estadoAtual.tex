...
(obrigatório). A pré-condição (linha 5) se inicia com a notação \texttt{/@pre} e
possui validação sobre o atributo descrição. Note que o atributo é precedido da
palavra \texttt{old}, que indica o valor do atributo antes do processamento do
...
t\'{i}picas do estilo arquitetural REST (chamadas 
a servi\c cos utilizando m\'{e}todos HTTP, sem\^{a}ntica de retorno 
das opera\c c\~{o}es observando os c\'{o}digos de retorno HTTP, etc.), o que 
habilita n\~{a}o apenas o estabelecimento de contratos mas tamb\'{e}m uma 
